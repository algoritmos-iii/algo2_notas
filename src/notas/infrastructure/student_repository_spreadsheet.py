...
from ..domain.student import Grade, StudentInfo, StudentWithGrades
...
class StudentRepositorySpreadsheet(
    ...
    def get_all(self) -> List[StudentWithGrades]:
        students_data_raw, exercises_data_raw = self._worksheet.batch_get(
            [self.STUDENT_DATA_RANGE, self.EXERCISES_RANGE],
    ...
    def get_student_by_padron(self, padron: int) -> Optional[StudentWithGrades]:
        ...
        try:
            return next(filter(lambda student: student.student_info.padron == padron, students))
